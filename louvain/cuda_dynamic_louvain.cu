// Code for cuda dynamic louvain (builds on static louvain)
// Implements: Static Louvain (returning reusable state),
//             Naive Dynamic Louvain, Frontier-based Dynamic Louvain,
    ...
    int node_to_move, int old_community, int new_community)
{
    ...
    double k_i_in_old = 0.0, k_i_in_new = 0.0, k_i = 0.0;
    ...
    int* d_affected)
{
    ...
    while (changed && louvain_kernel_iteration < 100) {
        ...
        for (long long i = ((long long)tid * m_edges) / nthreads; i < ((long long)(tid + 1) * m_edges) / nthreads; i++) {
            ...
            if (d_affected != nullptr && d_affected[node_to_move] == 0) continue;

            // Acquire vertex locks in sorted order to avoid deadlock
            ...
            if (initial_community != target_community) {
                ...
                double delta_Q = calculate_modularity_change(
                    n_nodes, m_edges, d_total_weight,
                    d_csr_adj, d_csr_node_offset,
                    community, community_degree,
                    node_to_move, initial_community, target_community);

                #ifdef DEBUG
                printf("Node %d: comm %d -> %d, delta_Q = %f\n",
                       node_to_move, initial_community, target_community, delta_Q);
                #endif

                if (delta_Q > 1e-12) {
                    atomicAdd((double*)&delta_Q_sum, delta_Q);
                    ...
                    if (d_affected != nullptr) {
                        for (int j = d_csr_node_offset[node_to_move];
                             j < d_csr_node_offset[node_to_move + 1]; j++) {
