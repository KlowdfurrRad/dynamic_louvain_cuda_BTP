    ...
    while(changed && louvain_kernel_iteration < 20) {
        ...
        for(int i = (tid * m_edges) / nthreads; i < ((tid + 1) * m_edges) / nthreads; i++) {
            ...
            if(initial_community != target_community) {
                ...
                double delta_Q = calculate_modularity_change(n_nodes, m_edges, d_total_weight, d_csr_adj, d_csr_node_offset, community, community_degree, d_vertex_weight, node_to_move, initial_community, target_community);
...
void louvain_cuda(vector<vector<pair<int, double>>> &adj, int n_nodes, int m_edges, const char* output_file) {
    ...
    double *d_vertex_weight;  // precomputed per-vertex degree snapshot; invariant during kernel
    ...
    cudaMalloc(&d_vertex_weight, n_nodes * sizeof(double));
    ...
    // Track original vertex -> community mapping through aggregation phases
    int* d_original_community;
    ...
    // Snapshot community_degree into d_vertex_weight. At this point in the flow,
    // every "community" is actually a singleton vertex (initial state or right
    // after aggregate_graph), so community_degree[v] == degree of vertex v.
    // The snapshot stays constant while the kernel mutates community_degree.
    cudaMemcpy(d_vertex_weight, d_community_degree, n_nodes * sizeof(double),
    ...
    // Download original community assignments and write to file
    int* h_original_community = new int[n_original_nodes];
    cudaMemcpy(h_original_community, d_original_community,
               n_original_nodes * sizeof(int), cudaMemcpyDeviceToHost);

    if (output_file) {
        set<int> unique_comms(h_original_community, h_original_community + n_original_nodes);
        ofstream fout(output_file);
        fout << unique_comms.size() << "\n";
        for (int i = 0; i < n_original_nodes; i++)
            fout << i << " " << h_original_community[i] << "\n";
        fout.close();
        cout << "Community assignments written to " << output_file << endl;
    }

    delete[] h_original_community;
...
int main(int argc, char* argv[]) {
    ...
    louvain_cuda(adj, n_nodes, m_edges * 2, output_file);
    return 0;
}