#!/usr/bin/env python3
"""Compact the per-method Louvain metrics in this directory's outputs/ into one CSV.

Scans outputs/<method>/<graph>.txt run logs (method = df / networkx / networkit /
cugraph) and writes results.csv -- one tidy row per (graph, method, variant, batch).
These are synthetic dynamic graphs (initial graph + 5 batches), so df contributes a
Static row plus ND/DF/DS rows per batch, and each baseline contributes a "rerun" row
per batch.

Log formats handled:
  df_louvain : "Static: Q=..  communities=..  passes=..  time=.. ms"
               "[batch N] ND/DF/DS: Q=..  comms=..  affected0=..  time=.. ms"
  nx/networkit/cugraph : "  initial : nodes=.. edges=.. Q=.. communities=.. time=..s"
                         "  batch N : nodes=.. edges=.. Q=.. communities=.. time=..s"

Columns: graph, nodes, edges, method, variant, batch, modularity, communities,
         passes, affected, time_s
  nodes/edges from graphs/<graph>.txt (the initial-graph header); time -> seconds.
...
def read_nm(path):
    """(nodes, edges) from the first line of the dynamic input, else (None, None)."""
