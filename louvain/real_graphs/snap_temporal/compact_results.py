#!/usr/bin/env python3
"""Compact the per-method Louvain metrics in snap_temporal/outputs/ into one CSV.

Scans snap_temporal/outputs/<method>/<graph>.txt run logs (method = df / networkx
/ cugraph) and writes snap_temporal/results.csv -- one tidy row per (graph, method,
variant, batch). These are DYNAMIC inputs (initial graph + batches), so df reports
Static + per-batch ND/DF/DS, and nx/cugraph re-run Louvain on the initial graph and
after each batch.

Log formats handled:
  df_louvain : "Static: Q=0.245513  communities=291  passes=3  time=123 ms"
...
Columns: graph, nodes, edges, method, variant, batch, modularity, communities,
  ...
  nodes/edges : the initial graph size (from converted/<graph>.txt)
...
def input_path(graph):
    return os.path.join(CONVERTED_DIR, f"{graph}.txt")
